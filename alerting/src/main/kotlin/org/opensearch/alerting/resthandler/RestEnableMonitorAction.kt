...
/**
 * Rest handlers to enable monitor
 */


class RestEnableMonitorAction : BaseRestHandler() {
    ...
    override fun routes(): List<RestHandler.Route> {
        return listOf(
            RestHandler.Route(
                ...
                "_plugins/_alerting/monitors/{monitor_id}/enable"
            )
        )
    }
    ...
    override fun prepareRequest(request: RestRequest, client: NodeClient): RestChannelConsumer {
        ...
        val enableMonitorRequest = EnableMonitorRequest(id, seqNo, primaryTerm, refreshPolicy, request.method(), monitor, rbacRoles)

        return RestChannelConsumer { channel ->
            val response = enableMonitorResponse(channel, request.method())
            client.execute(EnableMonitorAction.INSTANCE, enableMonitorRequest, response)
        }
    }
    ...
    private fun enableMonitorResponse(channel: RestChannel, restMethod: RestRequest.Method):
            RestResponseListener<EnableMonitorResponse> {
        return object : RestResponseListener<EnableMonitorResponse>(channel) {
            @Throws(Exception::class)
            override fun buildResponse(response: EnableMonitorResponse): RestResponse {
