...
import org.opensearch.alerting.util.*
...
private val log = LogManager.getLogger(TransportDisableMonitorAction::class.java)
private val scope: CoroutineScope = CoroutineScope(Dispatchers.IO)
class TransportDisableMonitorAction @Inject constructor(
...
) : HandledTransportAction<ActionRequest, DisableMonitorResponse>(
    AlertingActions.INDEX_MONITOR_ACTION_NAME, transportService, actionFilters, ::DisableMonitorRequest
),
    SecureTransportAction {

    @Volatile private var maxMonitors = AlertingSettings.ALERTING_MAX_MONITORS.get(settings)
    ...
    @Volatile private var maxActionThrottle = AlertingSettings.MAX_ACTION_THROTTLE_VALUE.get(settings)
    @Volatile private var allowList = DestinationSettings.ALLOW_LIST.get(settings)
    @Volatile override var filterByEnabled = AlertingSettings.FILTER_BY_BACKEND_ROLES.get(settings)

    init {
        clusterService.clusterSettings.addSettingsUpdateConsumer(AlertingSettings.ALERTING_MAX_MONITORS) { maxMonitors = it }
        clusterService.clusterSettings.addSettingsUpdateConsumer(AlertingSettings.REQUEST_TIMEOUT) { requestTimeout = it }
        clusterService.clusterSettings.addSettingsUpdateConsumer(AlertingSettings.INDEX_TIMEOUT) { indexTimeout = it }
        clusterService.clusterSettings.addSettingsUpdateConsumer(AlertingSettings.MAX_ACTION_THROTTLE_VALUE) { maxActionThrottle = it }
        clusterService.clusterSettings.addSettingsUpdateConsumer(DestinationSettings.ALLOW_LIST) { allowList = it }
        listenFilterBySettingChange(clusterService)
    }

    override fun doExecute(task: Task, request: ActionRequest, listener: ActionListener<DisableMonitorResponse>) {
        val transformedRequest = request as? DisableMonitorRequest
            ?: recreateObject(request, namedWriteableRegistry) {
                DisableMonitorRequest(it)
            }

        val user = readUserFromThreadContext(client)

        if (!validateUserBackendRoles(user, listener)) {
            return
        }
        ...
        if (!isADMonitor(transformedRequest.monitor)) {
            checkIndicesAndExecute(client, listener, transformedRequest, user)
        } else {
            // check if user has access to any anomaly detector for AD monitor
            checkAnomalyDetectorAndExecute(client, listener, transformedRequest, user)
        }
    }
    ...
    fun checkIndicesAndExecute(
        client: Client,
        actionListener: ActionListener<DisableMonitorResponse>,
        request: DisableMonitorRequest,
        user: User?,
    ) {
        val indices = mutableListOf<String>()
        // todo: for doc level alerting: check if index is present before monitor is created.
        val searchInputs = request.monitor.inputs.filter { it.name() == SearchInput.SEARCH_FIELD || it.name() == DocLevelMonitorInput.DOC_LEVEL_INPUT_FIELD }
            ...
            object : ActionListener<SearchResponse> {
                override fun onResponse(searchResponse: SearchResponse) {
                    ...
                    client.threadPool().threadContext.stashContext().use {
                        DisableMonitorHandler(client, actionListener, request, user).resolveUserAndStart()
                    }
                }
    ...
    fun checkAnomalyDetectorAndExecute(
        ...
        client.threadPool().threadContext.stashContext().use {
            DisableMonitorHandler(client, actionListener, request, user).resolveUserAndStartForAD()
        }
    }

    inner class DisableMonitorHandler(
        private val client: Client,
        private val actionListener: ActionListener<DisableMonitorResponse>,
        private val request: DisableMonitorRequest,
        ...
        private fun onCreateMappingsResponse(isAcknowledged: Boolean) {
            if (isAcknowledged) {
                ...
                log.info("Create ${ScheduledJob.SCHEDULED_JOBS_INDEX} mappings call not acknowledged.")
                actionListener.onFailure(
                    AlertingException.wrap(
                        OpenSearchStatusException(
        ...
        private suspend fun indexMonitor() {
            ...
            val indexRequest = IndexRequest(ScheduledJob.SCHEDULED_JOBS_INDEX)
                ...
                .source(request.monitor.toXContentWithUser(XContentFactory.jsonBuilder(), ToXContent.MapParams(mapOf("with_type" to "true"))))
        ...
        private suspend fun cleanupMonitorAfterPartialFailure(monitor: Monitor, response: IndexResponse) {
            try {
        ...
        private suspend fun onGetResponse(currentMonitor: Monitor) {
            ...
            request.monitor = request.monitor.copy(enabled = false, schemaVersion = IndexUtils.scheduledJobIndexSchemaVersion)
            val indexRequest = IndexRequest(ScheduledJob.SCHEDULED_JOBS_INDEX)
                .setRefreshPolicy(request.refreshPolicy)
                .source(request.monitor.toXContentWithUser(XContentFactory.jsonBuilder(), ToXContent.MapParams(mapOf("with_type" to "true"))))
            ...
            log.info(
                "Updating monitor, ${currentMonitor.id}, from: ${currentMonitor.toXContentWithUser(
                    XContentFactory.jsonBuilder(),
                    ToXContent.MapParams(mapOf("with_type" to "true"))
                )} \n to: ${request.monitor.toXContentWithUser(XContentFactory.jsonBuilder(), ToXContent.MapParams(mapOf("with_type" to "true")))}"
            )
