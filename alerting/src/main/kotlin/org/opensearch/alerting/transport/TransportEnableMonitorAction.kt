...
import org.opensearch.alerting.action.EnableMonitorRequest
import org.opensearch.alerting.action.EnableMonitorResponse
...
private val log = LogManager.getLogger(TransportEnableMonitorAction::class.java)
private val scope: CoroutineScope = CoroutineScope(Dispatchers.IO)
class TransportEnableMonitorAction @Inject constructor(
...
) : HandledTransportAction<ActionRequest, EnableMonitorResponse>(
    AlertingActions.INDEX_MONITOR_ACTION_NAME, transportService, actionFilters, ::EnableMonitorRequest
),
    ...
    override fun doExecute(task: Task, request: ActionRequest, listener: ActionListener<EnableMonitorResponse>) {
        val transformedRequest = request as? EnableMonitorRequest
            ?: recreateObject(request, namedWriteableRegistry) {
                EnableMonitorRequest(it)
            }
    ...
    fun checkIndicesAndExecute(
        client: Client,
        actionListener: ActionListener<EnableMonitorResponse>,
        request: EnableMonitorRequest,
        user: User?,
    ) {
            ...
            object : ActionListener<SearchResponse> {
                override fun onResponse(searchResponse: SearchResponse) {
                    ...
                    client.threadPool().threadContext.stashContext().use {
                        EnableMonitorHandler(client, actionListener, request, user).resolveUserAndStart()
                    }
                }
    ...
    fun checkAnomalyDetectorAndExecute(
        ...
        client.threadPool().threadContext.stashContext().use {
            EnableMonitorHandler(client, actionListener, request, user).resolveUserAndStartForAD()
        }
    }

    inner class EnableMonitorHandler(
        private val client: Client,
        private val actionListener: ActionListener<EnableMonitorResponse>,
        private val request: EnableMonitorRequest,
        ...
        private suspend fun onGetResponse(currentMonitor: Monitor) {
            ...
            request.monitor = request.monitor.copy(enabled = true, schemaVersion = IndexUtils.scheduledJobIndexSchemaVersion)
            val indexRequest = IndexRequest(ScheduledJob.SCHEDULED_JOBS_INDEX)
                .setRefreshPolicy(request.refreshPolicy)
                .source(request.monitor.toXContentWithUser(XContentFactory.jsonBuilder(), ToXContent.MapParams(mapOf("with_type" to "true"))))
